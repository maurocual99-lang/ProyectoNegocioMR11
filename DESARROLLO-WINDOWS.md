...
Si la PC ya tiene un servicio `postgresql-x64-*` de PostgreSQL 9.6 o posterior, MR11 usa esa instalación y su base `mr11` existente. Si no encuentra una contraseña válida en `C:\ProgramData\MR11\.env`, un formulario de MR11 pide la contraseña del usuario `postgres`; no la muestra en pantalla. El script verifica la conexión antes de crear o seleccionar la base `mr11`. Si hay varias instalaciones, prefiere el puerto guardado en `.env`; sin esa configuración pide elegir el servicio que contiene la base con datos. Si también existe el servicio `postgresql-mr11`, se utiliza ese servicio; comprobar primero que esa es la base que se quiere actualizar.

Los pedidos de contraseña y selección se muestran en ventanas gráficas porque el instalador NSIS no dispone de una consola interactiva confiable. PowerShell se ejecuta oculto y no debe aparecer una ventana negra; solamente se muestra el formulario de MR11 cuando hace falta una decisión del usuario. En una PC sin PostgreSQL, el usuario puede elegir y confirmar la contraseña o usar el botón para generar una segura. La contraseña queda guardada localmente en `C:\ProgramData\MR11\.env`; no se incluye una contraseña fija en el ejecutable. En una PC con PostgreSQL ya instalado se debe ingresar la contraseña actual de `postgres`: no es posible reemplazarla sin autenticarse primero.
...
Si `Get-Service postgresql-mr11` indica `Running` pero `Test-Path 'C:\ProgramData\MR11\.env'` devuelve `False`, hubo una instalación parcial. El instalador nuevo intenta recuperar localmente la contraseña del registro de PostgreSQL y, si no la encuentra, muestra una ventana para elegir una contraseña nueva. La recuperación habilita temporalmente una conexión local sin clave, cambia la contraseña y restaura inmediatamente `pg_hba.conf`; no borra el servicio ni los datos. Después verifica la conexión y completa la base. Si no puede hacerlo, deja el motivo en `instalacion-error.txt`.
